...
def from_pretrained(path: str, revision: str = None, **kwargs):
    ...
    Args:
        ...
        revision: The specific model version (commit hash, tag, or branch) to use.
    ...
    if is_local:
        ...
        logger.debug(f"Downloading model from HF Hub: repo={repo_id}, model={model_name}, revision={revision}")
        ...
        logger.debug(f"Downloaded config: {config_file}")
        logger.debug(f"Downloaded weights: {model_file}")
    ...
    logger.debug(f"Successfully loaded model {config['name']} from {path}")
